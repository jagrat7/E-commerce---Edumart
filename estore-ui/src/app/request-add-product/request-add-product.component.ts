...
@Component({
  selector: 'app-request-add-product',
  templateUrl: './request-add-product.component.html',
  styleUrls: ['./request-add-product.component.css']
})
export class RequestAddProductComponent implements OnInit {
  [x: string]: any;
    ...
    this.storeProductService.createProduct(product).subscribe(
      {
        next: (product) => {
          if(image == undefined){
            this.uploadFiles(product[product.type + "s"][0].id);
          }else{
            this.goBack();
          }
        }
  ...
  uploadFiles(id : Number) {
    ...
    if(this.selectedFiles.length != 0){
      this.storeProductService.uploadImage(this.selectedFiles[0], id, this.form.value.type).subscribe(
        {  
          next: (response) => {
